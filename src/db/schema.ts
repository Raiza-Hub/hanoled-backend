...
export const spreadsheetColumn = pgTable("spreadsheet_columns", {
  id: uuid("id").defaultRandom().primaryKey(),
  spreadsheetDetailsId: uuid("spreadsheet_details_id").references(
...
export const memberRelations = relations(member, ({ many, one }) => ({
  ...
  spreadsheetDetails: many(spreadsheetDetails),
  spreadsheetColumn: many(spreadsheetColumn),
}));
...
export const spreadsheetDetailsRelations = relations(
  spreadsheetDetails,
  ({ one, many }) => ({
    organization: one(organization, {
      fields: [spreadsheetDetails.organizationId],
      references: [organization.id],
    }),
    member: one(member, {
      fields: [spreadsheetDetails.memberId],
      references: [member.id],
    }),
    classLevel: one(classLevel, {
      fields: [spreadsheetDetails.classId],
      references: [classLevel.id],
    }),
    subject: one(subject, {
      fields: [spreadsheetDetails.subjectId],
...
export const spreadsheetColumnRelations = relations(
  spreadsheetColumn,
  ({ one }) => ({
    spreadsheetDetails: one(spreadsheetDetails, {
      fields: [spreadsheetColumn.spreadsheetDetailsId],
      references: [spreadsheetDetails.id],
    }),
  })
);
...
export type SpreadSheetDetails = typeof spreadsheetDetails.$inferSelect;

export type SpreadSheetColumn = typeof spreadsheetColumn.$inferSelect;
...
export const schema = {
  ...
  parentToStudents,
  subjectRelations,
  spreadsheetColumnRelations,
  spreadsheetDetailsRelations,
  spreadsheetDetails,
  spreadsheetColumn,
  // results,
};
